...
function snapToScale(
  ...
  for (let d = 1; d <= 6; d++) {
    if (midi + d <= ROLL_MIDI_TOP && midiInScale(midi + d, rootIdx, classes))
      return midi + d;
    if (midi - d >= ROLL_MIDI_BOTTOM && midiInScale(midi - d, rootIdx, classes))
      return midi - d;
  }
  return midi;
}
...
export function DetailPanel({
  ...
  useEffect(() => {
    ...
    function onClickOutside(e: MouseEvent) {
      if (
        ...
        !fxDropdownRef.current.contains(e.target as Node)
      ) {
  ...
  useEffect(() => {
    ...
    const centerMidi =
      loopNotes.length > 0 ? Math.max(...loopNotes.map((n) => n.note)) : 72; // default to C5
  ...
  const handleNoteMouseDown = useCallback(
    (e: React.MouseEvent, note: StudioNote) => {
      ...
      function onMove(ev: MouseEvent) {
        ...
        if (drag.type === "move") {
          const dStep = Math.round(
            (ev.clientX - drag.mouseStartX) / STEP_WIDTH,
          );
          const dMidi = -Math.round(
            (ev.clientY - drag.mouseStartY) / CELL_HEIGHT,
          );
          const newStep = Math.max(
            0,
            Math.min(drag.loopSteps - 1, drag.startStep + dStep),
          );
          let newMidi = Math.max(
            ROLL_MIDI_BOTTOM,
            Math.min(ROLL_MIDI_TOP, drag.startMidi + dMidi),
          );
          if (snapLock && dMidi !== 0)
            newMidi = snapToScale(newMidi, snapRootIdx, snapClasses);
          setDragPreview({
            noteId: drag.noteId,
            step: newStep,
            midi: newMidi,
            duration: drag.startDuration,
          });
          ...
          const newDur = Math.max(
            1,
            Math.min(maxDur, drag.startDuration + dStep),
          );
          setDragPreview({
            noteId: drag.noteId,
            step: drag.noteStep,
    ...
    (e: React.MouseEvent, note: StudioNote) => {
      ...
      function onMove(ev: MouseEvent) {
        ...
        const newVel = Math.max(
          0,
          Math.min(1, drag.startVelocity + delta / VEL_MAX_H),
        );
      ...
      function onUp() {
        ...
        if (drag.type === "velocity")
          onSetVelocity(drag.loopId, drag.noteId, drag.currentVelocity);
        dragRef.current = { type: "none" };
      }
      ...
      cancelDragRef.current = () => {
        ...
        const barEl = velBarRefs.current.get(noteId);
        if (barEl)
          barEl.style.height = `${Math.round(origVelocity * VEL_MAX_H)}px`;
      };
      ...
      <div className="studio-detail-header">
                ...
                >
                  ...
                  <Tooltip text="Remove effect">
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        if (loop) onRemoveFx(loop.id, entry.id);
                      }}
                    >
                      ×
                    </button>
        ...
        {loop?.syncMode === "sync_to" && (
          <select
            ...
            onChange={(e) =>
              onSetSyncMode(loop.id, "sync_to", e.target.value || null)
            }
          >
            {loops
        ...
        <div className="studio-scale-lock">
          ...
          <Tooltip text="Lock notes to selected scale">
            <button
          ...
          >
            ...
            <div className="studio-note-labels">
              {Array.from({ length: ROLL_NOTE_COUNT }, (_, i) => {
                ...
                return (
                  ...
                  >
                    {NOTE_NAMES[midi % 12] === "C" ||
                    NOTE_NAMES[midi % 12] === "F"
                      ? midiToName(midi)
                      : NOTE_NAMES[midi % 12]}
                  </div>
                );
            ...
            <div className="studio-roll-grid-wrapper" ref={scrollRef}>
              <div
                ...
                style={{
                  width: gridWidth,
                  minHeight: ROLL_NOTE_COUNT * CELL_HEIGHT,
                }}
              ...
              >
                ...
                {Array.from({ length: ROLL_NOTE_COUNT }, (_, i) => {
                  ...
                  const black = isBlackKey(midi);
                  const nonScale =
                    scaleLock && !midiInScale(midi, rootIdx, scaleClasses);
                  return (
                    <div
                      key={midi}
                      className={`studio-roll-row${black ? " black-key" : ""}${nonScale ? " non-scale" : ""}`}
                ...
                {isPlaying && (
                  <div
                    ...
                    style={{
                      left: ((currentStep % steps) / steps) * gridWidth,
                    }}
                  />
                )}
              </div>
...
const NoteBlock = memo(function NoteBlock({
  ...
  return (
    ...
    >
      ...
      {Object.keys(note.params).length > 0 && (
        <span className="studio-note-param-dot" />
      )}
...
interface VelocityLaneProps {
  ...
  steps: number;
  selectedNoteId: string | null;
  velBarRefs: React.RefObject<Map<string, HTMLDivElement>>;
...
const VelocityLane = memo(function VelocityLane({
  ...
  return (
    <div
      className="studio-velocity-lane"
      style={{ position: "relative", width: laneWidth + 40 }}
    >
      ...
      {notes.map((note) => (
        ...
        >
          <div
            ...
            style={{
              width: Math.max(4, STEP_WIDTH - 2),
              height: Math.round(note.velocity * VEL_MAX_H),
            }}
